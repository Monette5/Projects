...
var default_card_on_load = function (data) {
	...
	//console.log(settings.image0);
	//console.log(settings.image1);
...
//console.log(data.image0);
    if(Wix.Utils.getViewMode()=='site'){
		...
		{
            $("#face0").css("background-color", "transparent");

			//$("#face0").addClass('undefined1');
		}

		if(settings.image1 != undefined)
		{
			$("#face0").css("background-color", "transparent");
			//$("#face0").addClass('undefined2');
		}
...
if(settings.image0 == undefined)
		{
			$("#face0").css('background', 'none!important');
		}
...
var default_particles_on_load = function (data){
        ...
        if(data.transitiontype=="particles"){
            ...
            if(data.endStateLink==true || data.endStateLink=='true'){
                $("#particles ").removeClass('target target-link');
                $('.particles-button').off().on('click',function(){
                    setTimeout(function(){
                        console.log('dfdfdf');
                        $("#particles").addClass('target target-link');
...
var default_distortion_on_load = function (data){
    ...
    function getImage(){
        if(data.image0[0]!=undefined){
             image0Src = Wix.Utils.Media.getResizedImageUrl(data.image0[0].relativeUri, $(window).width(), $(window).height());
        }
        if(data.image1[0]!=undefined){
             image1Src = Wix.Utils.Media.getResizedImageUrl(data.image1[0].relativeUri, $(window).width(), $(window).height());
        }
        $('.grid__item-img  .dist_editor_img').css('width',$(window).width()+'px');
        $('.grid__item-img  .dist_editor_img').css('height',$(window).height()+'px');

        console.log(image0Src);
        ...
        if(Wix.Utils.getViewMode()=='editor'){
            $('.grid__item-img  .dist_editor_img').removeClass('hide');
                if(data.startEndToggle==1){
                    $('.grid__item-img  .dist_editor_img').css('background-image', 'url("' + image0Src + '")');

                }else{
                    $('.grid__item-img  .dist_editor_img').css('background-image', 'url("' + image1Src + '")');
                }
    ...
    if(data.subheading0!=undefined){
        $(".grid__item-subtitle span").text(data.subheading0);
    } else{
        $(".grid__item-subtitle span").text("Wow your users with stunning animations");
    }

    if(data.heading0!=undefined){
        $(".grid__item-title").text(data.heading0);
    } else{
        $(".grid__item-title").text("Welcome to rollover");
    }
    $(".grid__item-subtitle .grid__item-link").text(data.textdistlink);
...
var default_distortion_on = function (data){
    ...
    console.log('default_distortion_on');

}

var default_distortion_off = function (data){
    console.log('default_distortion_off');
}


// var default_tilter_on_load = function (data){
//
//     //var img0 = Wix.Utils.Media.getResizedImageUrl(data.image0[0].relativeUri, 500, 500);
//     var img0 = Wix.Utils.Media.getResizedImageUrl(settings.image0[0].relativeUri, 300, 415);
//     var txt0 = data.heading0;
//     var txt1 = data.subheading0;
//     console.log(img0);
//     $('.tilter__image').attr("src", img0);
//    $("#tilter .tilter__title").html(parseText(txt0))
//    $("#tilter .tilter__description").html(parseText(txt1));
// }
//
// var default_tilter_on = function (data){
//
// }
//
// var default_tilter_off = function (data){
//
// }
var default_tilter_on_load = function (data){
  var tilterclass = $('#tilter .tilter').attr("class").match(/\d+/g)[0];
  var img0 = Wix.Utils.Media.getResizedImageUrl(settings.image0[0].relativeUri, 300, 415);
  var txt0 = data.heading0;
  var txt1 = data.subheading0;
    ...
    switch (data.transition) {
    case 'tilter-1':
     setTilters(tilterclass,'tilter--1', img0);
     break;
     case 'tilter-2':
     setTilters(tilterclass,'tilter--2', img0);
     break;
     case 'tilter-3':
      setTilters(tilterclass,'tilter--3', img0);
     break;
     case 'tilter-4':
     setTilters(tilterclass,'tilter--4', img0);
     break;
     case 'tilter-5':
     setTilters(tilterclass,'tilter--5', img0);
     break;
     case 'tilter-6':
     setTilters(tilterclass,'tilter--6', img0);
     break;
     case 'tilter-7':
    setTilters(tilterclass,'tilter--7', img0);
    break;
    case 'tilter-8':
    setTilters(tilterclass,'tilter--8', img0);
      break;
  function setTilters(tilterclass,currentTilter,img0){
      $('#tilter .tilter').removeClass("tilter-"+tilterclass).addClass(currentTilter);
      $('#tilter-image0').attr("src", img0);
      $("#tilter .tilter__title").html(parseText(txt0))
  		$("#tilter .tilter__description").html(parseText(txt1));
      //$("#tilter .tilter").css('background-image', 'url(' + img0 + ')');//May change to transparent
      $('#svgTilter').css({
    'width': $('#tilter .tilter').width() + 'px',
    'height': $('#tilter .tilter').height() + 'px'
});
          if (data.noimage0 == 'true' || data.noimage0 == 1) {
              $('#tilter .tilter__image').css('background', 'linear-gradient(' + data.overlayOffAngle + 'deg,' + data.colorFilterOn1 + ', ' + data.colorFilterOn2 + ')');
          }
