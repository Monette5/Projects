...
/********Particle Buttons for test only, not finished******************/
var default_particles_on_load = function (data){

    if(data.particlescolor){
        $('.particles-button').css('fill',data.particlescolor);
    }
    $("#particles .grid").css('grid-auto-rows',(heightContainer/2)+'px');

    if(heightContainer > '300' && widthContainer > '300'){
        $(".particles-button").css('min-height','150px');
    }

    if(getParameterByName('viewMode') == "editor"){
    $( "div" ).remove( ".svg-layer" );
        if(document.querySelector(".particles-button svg")!=null ){
    $(' #forcestyle2').html('.particles-button .svg-layer{-webkit-mask-image : url("../src/widget/images/' + data.transitiontype + '/' + data.transition + '.svg") no-repeat center !important;  mask-image : url("../src/widget/images/' + data.transitiontype + '/' + data.transition + '.svg") no-repeat center !important; }');
            }else{
                $(".particles-button div.svg-layer").replaceWith('<div class=".grid__item"></div>');
            }
        }else{
            $(".svg-layer").remove();
            $(".particles-button").append('<div class="svg-layer"></div>');
            svgLayerParticles();
        }
        $(".particles-button").css('opacity','1');
        $(".particles-button").css('transform','scale(1) rotate(0deg)');


    function svgLayerParticles(){
        $(' #forcestyle2').html('.particles-button .svg-layer{ -webkit-mask: url("../src/widget/images/' + data.transitiontype + '/' + data.transition + '.svg") no-repeat center;mask: url("../src/widget/images/' + data.transitiontype + '/' + data.transition + '.svg") no-repeat center;}');
        $('..particles-button .svg-layer').css('background-color', data.particlescolor);
}
      }
...
var default_cooltips_on_load = function (data){
    ...
    var heightContainer     = $("#cooltips .content--intro").height();
    var widthContainer      = $("#cooltips .content--intro").width();
    ...
    $("#cooltips .grid").css('grid-auto-rows',(heightContainer/2)+'px');

    if(heightContainer > '300' && widthContainer > '300'){
        $(".tooltip__base").css('min-height','185px');
    }
